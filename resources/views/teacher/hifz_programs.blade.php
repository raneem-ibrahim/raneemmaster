...
.btn-black:focus:active {
    ...
    color: #fff !important;
        ...
        <h4 class="text-primary" style="color: #c37044 !important;">جدول الأسبوع ابتداءً من: {{ \Carbon\Carbon::parse($program->week_start)->format('Y-m-d') }}</h4>
    ...
    <div class="modal-content border border-dark-subtle">
      <form action="{{ route('teacher.day.update', $day->id) }}" method="POST">
        ...
        <div class="modal-footer border-top">
          <button type="submit" class="btn " style="background-color:  #c37044 ; color: white;"> حفظ التعديلات</button>
          <button type="button" class="btn " data-bs-dismiss="modal"  style="background-color:  #040302 ; color: white;"> إلغاء</button>
        </div>
      </form>
    </div>
  </div>
